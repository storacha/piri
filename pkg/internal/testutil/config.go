...
type TestConfigOption func(testing.TB, *app.AppConfig)
...
func NewTestConfig(t testing.TB, opts ...TestConfigOption) app.AppConfig {
	t.Helper()

	// Start with sensible defaults for testing
	cfg := app.AppConfig{
		Identity: app.IdentityConfig{
			Signer: Alice, // Default test signer
		},
		Server: app.ServerConfig{
			Host:      "localhost",
			Port:      8080,
			PublicURL: Must(url.Parse("http://localhost:8080"))(t),
		},
		...
		External: app.ExternalServicesConfig{
			UploadService: app.ServiceConnectionConfig{
				Connection: Must(client.NewConnection(presets.UploadServiceDID, ucanhttp.NewHTTPChannel(presets.UploadServiceURL)))(t),
			},
		},
		Services: app.ServicesConfig{
			Publisher: app.PublisherConfig{
				PublicMaddr:   Must(multiaddr.NewMultiaddr("/ip4/127.0.0.1/tcp/8080/http"))(t),
				AnnounceMaddr: Must(multiaddr.NewMultiaddr("/ip4/127.0.0.1/tcp/8080/http"))(t),
...
func WithSigner(signer principal.Signer) TestConfigOption {
	return func(_ testing.TB, cfg *app.AppConfig) {
...
func WithUploadServiceURL(uploadURL *url.URL) TestConfigOption {
	return func(t testing.TB, cfg *app.AppConfig) {
		if uploadURL != nil {
			...
			cfg.External.UploadService.Connection = Must(client.NewConnection(
				did,
