...
func newTestStore() *Store[testItem] {
	backend := dsadapter.New(datastore.NewMapDatastore())
	return New[testItem](backend, "test/", jsonCodec{})
}
...
func TestGenericStore(t *testing.T) {
	...
	t.Run("prefix isolation", func(t *testing.T) {
		backend := dsadapter.New(datastore.NewMapDatastore())
		store1 := New[testItem](backend, "prefix1/", jsonCodec{})
		store2 := New[testItem](backend, "prefix2/", jsonCodec{})
