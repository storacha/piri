...
func BlobAcceptHandler(service BlobAcceptService) server.Option {
	return server.WithServiceMethod(
		blob.AcceptAbility,
		server.Provide(
			blob.Accept,
			func(
				cap ucan.Capability[blob.AcceptCaveats],
				...
				ctx := context.TODO()
				resp, err := blobhandler.Accept(ctx, service, &blobhandler.AcceptRequest{
					Space: cap.Nb().Space,
					Blob:  cap.Nb().Blob,
					Put:   cap.Nb().Put,
				})
