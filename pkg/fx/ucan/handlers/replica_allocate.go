...
func ReplicaAllocateHandler(service ReplicaAllocateService) server.Option {
	return server.WithServiceMethod(
		...
		server.Provide(
			...
			func(
				...
				ctx := context.TODO()
				resp, err := blobhandler.Allocate(ctx, service, &blobhandler.AllocateRequest{
					Space: cap.Nb().Space,
					Blob:  cap.Nb().Blob,
					Cause: inv.Link(),
				})
				...
				if err := service.Replicator().Replicate(ctx, &replicahandler.TransferRequest{
					Space:  cap.Nb().Space,
					Blob:   cap.Nb().Blob,
