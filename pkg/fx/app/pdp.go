...
func ProvideEthClient(lc fx.Lifecycle, cfg app.AppConfig) (*ethclient.Client, error) {
	ethAPI, err := ethclient.Dial(cfg.PDPService.Local.LotusEndpoint.String())
	if err != nil {
...
func ProvideLotusClient(lc fx.Lifecycle, cfg app.AppConfig) (api.FullNode, error) {
	lotusAPI, closer, err := client.NewFullNodeRPCV1(context.TODO(), cfg.PDPService.Local.LotusEndpoint.String(), nil)
	if err != nil {
