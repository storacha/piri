...
func NewStores(cfg app.StorageConfig) (*Stores, error) {
	...
	if stores.Egress, err = minio_store.New(endpoint, prefix+"egress", options); err != nil {
		return nil, fmt.Errorf("creating egress s3 store: %w", err)
	}
...
func NewAllocationStore(stores *Stores) allocationstore.AllocationStore {
	return allocationstore.NewMinioAllocationStore(stores.Allocations)
}

func NewAcceptanceStore(stores *Stores) acceptancestore.AcceptanceStore {
	return acceptancestore.NewMinioAcceptanceStore(stores.Acceptances)
}

func NewBlobStore(stores *Stores) blobstore.Blobstore {
	return blobstore.NewObjectBlobstore(stores.Blobs)
}

func NewClaimStore(stores *Stores) (claimstore.ClaimStore, error) {
	return delegationstore.NewMinioDelegationStore(stores.Claims)
}

func NewReceiptStore(stores *Stores) (receiptstore.ReceiptStore, error) {
	return receiptstore.NewMinioReceiptStore(stores.Receipts)
}

func NewRetrievalJournal(stores *Stores, svcCfg app.UCANServiceConfig, lc fx.Lifecycle) (retrievaljournal.Journal, error) {
	rj, err := retrievaljournal.NewMinioJournal(stores.Egress, svcCfg.Services.EgressTracker.MaxBatchSizeBytes)
	if err != nil {
		return nil, fmt.Errorf("creating s3 retrieval journal: %w", err)
	}
	lc.Append(fx.Hook{
		OnStop: func(ctx context.Context) error {
			return rj.Close()
...
func NewPDPStore(stores *Stores) blobstore.PDPStore {
	return blobstore.NewObjectBlobstore(stores.PDP)
}

func NewConsolidationStore(stores *Stores) consolidationstore.Store {
	return consolidationstore.NewMinioConsolidationStore(stores.Consolidation)
}
