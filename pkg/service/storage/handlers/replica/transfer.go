...
//
// This design addresses a critical issue where network failures during receipt generation
// (after successful transfer) would cause the entire operation to retry, leading to
// redundant transfers of potentially large blobs. By checking existence first, we ensure
// that subsequent retries recognize the blob is already stored and skip directly to
// receipt generation.
//
...
func Transfer(ctx context.Context, service TransferService, request *TransferRequest) error {
	...
	// Check if the blob already exists - critical for idempotency
	...
	return sendMessageToUploadService(ctx, service, msg)
}
...
func sendMessageToUploadService(ctx context.Context, service TransferService, msg message.AgentMessage) error {
...
// TransferReplicaBlobError represents a transfer operation failure that can be encoded to IPLD
type TransferReplicaBlobError struct {
	message string
}

func (tf TransferReplicaBlobError) Name() string {
	return "TransferReplicaBlobError"
}

func (tf TransferReplicaBlobError) Error() string {
	return tf.message
}

func (tf TransferReplicaBlobError) ToIPLD() (ipld.Node, error) {
	name := tf.Name()
	model := struct {
	...
	return ipld.WrapWithRecovery(&model, nil)
}
...
func SendFailureReceipt(ctx context.Context, service TransferService, request *TransferRequest, transferErr error) error {
	// Create a failure that implements IPLDBuilder
	failure := TransferReplicaBlobError{
		message: fmt.Sprintf("Transfer failed after all retries: %v", transferErr),
	}

	// Create an error result for the transfer
	errResult := result.Error[replica.TransferOk, TransferReplicaBlobError](failure)

	// Issue the failure receipt
	...
	// Store the failure receipt
	if err := service.Receipts().Put(ctx, rcpt); err != nil {
		return fmt.Errorf("failed to store failure receipt: %w", err)
	}

	// Create agent message with the failure receipt
	agentMsg, err := message.Build([]invocation.Invocation{}, []receipt.AnyReceipt{rcpt})
	if err != nil {
		return fmt.Errorf("building failure message: %w", err)
	}

	// Send the failure receipt to the upload service
	if err := sendMessageToUploadService(ctx, service, agentMsg); err != nil {
