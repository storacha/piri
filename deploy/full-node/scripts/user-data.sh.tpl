...
cat > /etc/piri/config.toml <<'EOF'
${config_toml_content}
EOF
chown piri:piri /etc/piri/config.toml
...
# Import wallet before starting service
echo "=== Importing wallet ==="
sudo -u piri /usr/local/bin/piri --config=/etc/piri/config.toml wallet import /etc/piri/wallet.hex
